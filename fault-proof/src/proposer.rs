    ...
    P: Provider + Clone + Send + Sync + 'static,
{
    ...
    async fn sync_anchor_game(&self) -> Result<()> {
        let anchor_root_l2_block_number =
            self.anchor_state_registry.getAnchorRoot().call().await?._1;
        let anchor_game_address = self.anchor_state_registry.anchorGame().call().await?;
    ...
    pub async fn prove_game(
        ...
        let l1_head_hash = B256::from(game.l1Head().call().await?.0);
    ...
    async fn check_game_claim_eventual_validity(
        &self,
        game_address: Address,
    ) -> anyhow::Result<Option<GameEventualValidityError>> {
    ...
    async fn should_create_game_with_range_optimizer(&self) -> Result<(bool, U256, u32)> {
        ...
        // Create providers
        let l1_chain_provider = AlloyChainProvider::new(self.l1_provider.clone(), 1024);
        ...
        let beacon_client = OnlineBeaconClient::new_http(
            self.fetcher
                ...
                .l1_beacon_rpc
                .clone()
                .context("L1 beacon RPC must be set")?
                .to_string(),
        );
        let blob_provider = OnlineBlobProvider::init(beacon_client).await;

        let l2_chain_provider =
            AlloyL2ChainProvider::new(self.l2_provider.clone(), rollup_config.clone(), 1024);
