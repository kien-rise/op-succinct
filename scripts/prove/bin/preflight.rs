...
async fn main() -> Result<()> {
    ...
    let endpoint_url = Url::parse(&endpoint).expect("Failed to parse endpoint URL");
    let rpc_client =
        kona_host::eth::rpc_client(endpoint_url, l1_requests_per_second, l1_max_retries)?;
    ...
    let provider_with_signer =
        ProviderBuilder::new().wallet(EthereumWallet::from(wallet)).on_client(rpc_client);
